...
pub struct FuseFilesystem<F: super::Filesystem> {
    fs: F,
    rt: Runtime,
}

impl<F: super::Filesystem> FuseFilesystem<F> {
    pub fn new(fs: F, rt: Runtime) -> Self {
        Self { fs, rt }
    }
}
...
impl<F: super::Filesystem> fuser::Filesystem for FuseFilesystem<F> {
    fn lookup(
        &mut self,
        req: &fuser::Request<'_>,
        parent: u64,
