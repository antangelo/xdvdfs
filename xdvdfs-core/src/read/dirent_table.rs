...
enum DirentSearchResult {
    Found(DirectoryEntryNode),
    NextOffset(u64),
}

impl DirectoryEntryTable {
    #[maybe_async]
    async fn dirent_search_next_offset<BDR: BlockDeviceRead + ?Sized>(
        &self,
        dev: &mut BDR,
        name: &str,
        offset: u64,
    ) -> Result<DirentSearchResult, util::Error<BDR::ReadError>> {
        ...
        let next_offset = match cmp {
            core::cmp::Ordering::Equal => return Ok(DirentSearchResult::Found(dirent)),
    ...
    async fn find_dirent<BDR: BlockDeviceRead + ?Sized>(
        ...
        loop {
            match self.dirent_search_next_offset(dev, name, offset).await? {
                DirentSearchResult::Found(dirent) => break Ok(dirent),
...
mod test {
    ...
    #[test]
    fn test_read_dirtab_dirent_next_offset_empty_dirent() {
        let mut dev = [0xffu8; 0x10];
        ...
        let res = block_on(table.dirent_search_next_offset(dev.as_mut_slice(), "a", 0));
        assert!(matches!(res, Err(util::Error::DoesNotExist)));
    }
    ...
    fn test_read_dirtab_dirent_next_offset_found_match() {
        ...
        let res = block_on(table.dirent_search_next_offset(dev.as_mut_slice(), "Bc", 0))
        ...
        assert_eq!(
            res,
            DirentSearchResult::Found(DirectoryEntryNode {
    ...
    fn test_read_dirtab_dirent_next_offset_left_child() {
        ...
        let res = block_on(table.dirent_search_next_offset(dev.as_mut_slice(), "ba", 0))
            .expect("Dirent search should succeed");
    ...
    fn test_read_dirtab_dirent_next_offset_right_child() {
        ...
        let res = block_on(table.dirent_search_next_offset(dev.as_mut_slice(), "bd", 0))
            .expect("Dirent search should succeed");
        assert_eq!(res, DirentSearchResult::NextOffset(4 * 514));
    }

    #[test]
    fn test_read_dirtab_dirent_next_offset_zero_child_does_not_exist() {
        #[rustfmt::skip]
        let mut dev = [
            1, 1, 0, 0,
            1, 0, 0, 0,
    ...
    fn test_read_dirtab_find_dirent_empty_zero_size() {
        ...
        assert!(matches!(res, Err(util::Error::DirectoryEmpty)));
    }
    ...
    fn test_read_dirtab_walk_path_root() {
        ...
        assert!(matches!(res, Err(util::Error::NoDirent)));
    }
