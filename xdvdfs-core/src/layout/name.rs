...
impl<'alloc> DirentName<'alloc> {
    ...
    pub fn set_encode_name(&mut self) -> Result<u8, crate::write::FileStructureError> {
        ...
        match result {
            ...
            EncoderResult::OutputFull => return Err(FileStructureError::FileNameTooLong),
            EncoderResult::Unmappable(_) => return Err(FileStructureError::StringEncodingError),
        }
...
mod test {
    ...
    fn test_layout_dirent_name_encode_over_max_len() {
        ...
        let mut name = DirentName::new(&s);
        let res = name.set_encode_name();
        assert_eq!(res, Err(FileStructureError::FileNameTooLong));
    }
    ...
    fn test_layout_dirent_name_encode_invalid_char() {
        ...
        let res = name.set_encode_name();
        assert_eq!(res, Err(FileStructureError::StringEncodingError));
    }
}
